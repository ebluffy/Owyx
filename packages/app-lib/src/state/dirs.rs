...
/// Curated Owyx server packs live under `profiles/servers/` (not the Library root).
...
impl DirectoryInfo {
    ...
    /// Curated Owyx server pack instances (`profiles/servers/`).
    #[inline]
    pub fn servers_dir(&self) -> PathBuf {
        self.instances_dir().join(SERVERS_FOLDER_NAME)
    }
